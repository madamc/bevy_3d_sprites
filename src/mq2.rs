...
pub fn fill_mando_queue2(
    ...
    let plyr_location = set.p0().single().1.0;
    let plyr_entity = set.p0().single().0;
    ...
    mando_queue.mandos.push_back(mando!(move_to_loc_2d_iv2_cmd; vec![
                                                                    Mp::Float(1.), 
                                                                    Mp::IVec2(plyr_location), 
                                                                    Mp::IVec2(IVec2 {x: 30, y:YNYN_Y}),
                                                                    Mp::BevyEntity(plyr_entity)]));
    ...
    mando_queue.mandos.push_back(mando!(type_writer_effect_cmd; vec![Mp::String(Message1.to_owned())]));

    mando_queue.mandos.push_back(mando!(type_writer_effect_cmd; vec![Mp::String(Message2.to_owned())]));
